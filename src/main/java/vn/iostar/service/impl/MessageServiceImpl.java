...
public class MessageServiceImpl implements MessageService {
    ...
    public ResponseEntity<GenericResponse> getListMessageByUserIdAndUserTokenId(String userId, String userIdToken,
        ...
        return ResponseEntity.ok(GenericResponse.builder().success(true).message("Get MessageDTO successfully").result(messages)
                .statusCode(HttpStatus.OK.value()).build());
    }

    @Override
    public ResponseEntity<GenericResponse> getListMessageByGroupIdAndUserTokenId(String groupId,
                                                                                 PageRequest pageable) {
        ...
        List<MessageDTO> messages = messageRepository.findByGroupPostGroupIdOrderByCreateAt(Integer.parseInt(groupId), pageable);
        if (messages.isEmpty())
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(GenericResponse.builder().success(false)
                    .message("Not found message").statusCode(HttpStatus.NOT_FOUND.value()).build());

        return ResponseEntity.ok(GenericResponse.builder().success(true).message("Get list Message successfully").result(messages)
                .statusCode(HttpStatus.OK.value()).build());
    }
    ...
    public Message saveMessageByDTO(MessageDTO message) throws IOException {
        Optional<Message> message1 = messageRepository.findByCreateAtAndSenderUserIdAndReceiverUserIdAndContent(message.getCreatedAt(),
