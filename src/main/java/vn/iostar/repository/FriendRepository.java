...
public interface FriendRepository extends JpaRepository<Friend, Integer> {
	...
	@Query("SELECT DISTINCT NEW vn.iostar.dto.FriendRequestResponse(u.userId,u.profile.background, u.profile.avatar, u.userName) FROM Friend f "
	...
	List<FriendRequestResponse> findFriendUserIdsByUserId(@Param("userId") String userId);

	// Get List userId by UserID where become friend
	@Query("SELECT DISTINCT NEW vn.iostar.dto.FriendRequestResponse(u.userId,u.profile.background, u.profile.avatar, u.userName) FROM Friend f "
			+ "JOIN f.user1 u1 " + "JOIN f.user2 u2 " + "JOIN User u ON u = u1 OR u = u2 "
			+ "WHERE (u1.userId = :userId OR u2.userId = :userId) " + "AND u.userId <> :userId")
	List<FriendRequestResponse> findFriendTop10UserIdsByUserId(@Param("userId") String userId, Pageable pageable);
